...
def _get_path(path):
    return path.rstrip(os.pathsep) + os.altsep
...
class Sophie:
    ...
    def run(self):
        ...
        if enable_vhost_creation:
            replacements = {'{vhost}': self.vhost,
                            '{vhost_document_root}': _get_path(http_www_path) + _get_public_path(http_public_path)}
            self.create_vhost_dir()
            self.create_vhost_conf(replacements)

        if enable_git_creation:
            replacements = {'{vhost_path}': _get_path(http_www_path) + self.repo + os.altsep + _get_path(git_checkout_path),
    ...
    def create_repo_dir(self):
        ...
        print("Creating %s" % repo_path)
        os.makedirs(repo_path)

        # TODO this should be created from git init --bare
        print("Creating %s" % repo_path + os.altsep + 'hooks')
        os.makedirs(repo_path + os.altsep + 'hooks')
    ...
    def create_repo_conf(self, replacements):
        ...
        hooks_path = _get_path(git_repo_base_path) + self.repo + os.altsep + 'hooks' + os.altsep
